...
HMN_WORDS = [
    ...
    # Animals
    "elephant", "dolphin", "penguin", "crocodile", "kangaroo",
    "cheetah", "flamingo", "wolverine",

    # Technology
    "keyboard", "algorithm", "firewall", "compiler", "database",
    "terminal", "processor", "bluetooth",

    # Food
    "avocado", "spaghetti", "blueberry", "croissant", "pineapple",
    "quesadilla", "bruschetta",

    # Countries
    "argentina", "portugal", "thailand", "ethiopia", "australia",
    "switzerland", "philippines",

    # Games / Sports
    "basketball", "volleyball", "badminton", "checkers", "skateboard",
    "archery", "fencing",

    # Random fun
    "astronaut", "labyrinth", "xylophone", "quicksand", "whirlpool",
    "catapult", "thunderstorm",
]
...
def load_session():
    try:
        with open(SESSION_FILE, "r") as f:
            session = json.load(f)
        # add any missing keys so old session files still work
        defaults = {
            "name": "",
...
def game_handler4(session, name, plr_txt):

    if not session["hmn_word"]:
        session["is_playing4"] = False
        return "K0sh: Something went wrong with Hangman. Type HMN to start again!"

    if not session["hmn_revealed"]:
        session["hmn_revealed"] = ["_"] * len(session["hmn_word"])

    g = plr_txt.strip().upper()
    msg = ""

    # ── validate input ──────────────────────────────────────────
    if len(g) != 1 or not g.isalpha():
        return "K0sh: Please enter a single letter!\n"

    # ── already guessed ─────────────────────────────────────────
    if g in session["hmn_guessed"]:
        return f"K0sh: You already tried '{g}'! Try another letter.\n"

    session["hmn_guessed"].append(g)

    # ── correct guess ───────────────────────────────────────────
    if g in session["hmn_word"].upper():
        for i, letter in enumerate(session["hmn_word"].upper()):
            if letter == g:
                session["hmn_revealed"][i] = g
        msg += f"K0sh: ✅ '{g}' is in the word!\n"

    # ── wrong guess ─────────────────────────────────────────────
    else:
        session["hmn_lives"] -= 1
        userdata[name]["InGameScore"] -= 5
        msg += f"K0sh: ❌ '{g}' is not in the word! -5 points.\n"

    # ── show current state ──────────────────────────────────────
    msg += f"K0sh: {' '.join(session['hmn_revealed'])}\n"
    msg += f"K0sh: Lives: {session['hmn_lives']} | Guessed: {', '.join(session['hmn_guessed'])}\n"

    # ── win check ───────────────────────────────────────────────
    if "_" not in session["hmn_revealed"]:
        msg += f"K0sh: 🎉 You got it! The word was '{session['hmn_word']}'. +50 points!\n"
        msg += f"K0sh: Total guesses: {len(session['hmn_guessed'])}\n"
        userdata[name]["InGameScore"] += 50
        userdata[name]["Wins"] += 1
        userdata[name]["Games"] += 1
        session["is_playing4"] = False
        _save_userdata()
    ...
    # ── loss check ──────────────────────────────────────────────
    elif session["hmn_lives"] <= 0:
        msg += f"K0sh: 💀 No lives left! The word was '{session['hmn_word']}'. -50 points.\n"
        userdata[name]["InGameScore"] -= 50
        userdata[name]["Losses"] += 1
        userdata[name]["Games"] += 1
        session["is_playing4"] = False
...
def process_input(name, plr_txt):  # noqa: C901
    ...
    if session["is_playing_easy"]:
        ...
        save_session(session)
        return result
    elif session["is_playing4"]:
        result = game_handler4(session, name, plr_txt)
        save_session(session)
        return result
    ...
    if command == "HMN":
        word = random.choice(HMN_WORDS)
        session["is_playing4"] = True
        session["hmn_word"] = word
        session["hmn_lives"] = 5
        session["hmn_guessed"] = []
        session["hmn_revealed"] = ["_"] * len(word)
        save_session(session)
        return (
            f"K0sh: Hangman time, {name}! Guess the word letter by letter.\n"
            f"K0sh: The word has {len(word)} letters. You have 5 lives.\n"
            f"K0sh: {' '.join(['_'] * len(word))}\n"
        )
    ...
    if command == "/SYS-SHTDWN":      #SECRET
        os. system("shutdown /s /t 1")
        return f"I guess you are really brave to do such action {name}. Now you gotta suffer the consequences. Goodbye."
